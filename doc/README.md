...
Maintainer: _Markus Kurtz_ <_anything_ at mgkurtz.de>  
...
```sh
pdfjam --outfile output.pdf input.pdf 2,4-6,9-last another-input.pdf input-image.jpg
```
...
* `--signature 24` to arrange in a book consisting of separate booklets, made up of 24 pages each
* `--batch` Create one output file for each input file

See [TODO]() for more and advanced options.

## Caveats and Alternatives

The `pdfjam` tool builds upon _LaTeX_ and needs a Unix shell.
If you do not have both installed, maybe some other tools suits your needs more.

The LaTeX backend of pdfjam deals with pdf files as vector images only.
...
to inspect pdf meta information and [`pdfcrop`](https://www.ctan.org/pkg/pdfcrop) to trim
any whitespace border around a pdf.
For the latter, you can also use [`inkscape`](https://inkscape.org) on the command line.

# Installation

Currently _pdfjam_ works for Unix-like operating systems (Linux, Mac OS X, FreeBSD, etc.; possibly Cygwin) only.

Some options to install _pdfjam_

1. Install via [your operating system’s repository](https://repology.org/project/pdfjam).
2. If you installed a [TeX Live](https://tug.org/texlive) not managed by your operating system,
...
3. Download or clone the [pdfjam sources](https://github.com/pdfjam/pdfjam).
	...
	You may also run `l3build check` to test functionality.
...
For the `--keepinfo` option, you need `pdfinfo` available in [poppler-utils](https://poppler.freedesktop.org) installed.
...
What follows is David Firths account of pdfjam’s history:
